...
$sql = mysqli_query($con,"SELECT * FROM $prefix WHERE target = $session->uid AND archived = 1 ORDER BY time DESC");
...
$sql2 = mysqli_query($con,"SELECT * FROM $prefix WHERE target = $session->uid AND archived = 1 ORDER BY time DESC $limit");
$paginationDisplay = "";
