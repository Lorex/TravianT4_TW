<?php
	...
	$sql = mysqli_query($con,"SELECT * FROM ".TB_PREFIX."raidlist WHERE lid = ".$lid."");
	while($row = mysqli_fetch_array($sql)){
		...
		if($slots[$sid]=='on'){
			...
			if($database->checkVilExist($data['to_vid'])){
				$query1 = mysqli_query($con,'SELECT * FROM `' . TB_PREFIX . 'vdata` WHERE `wref` = ' . $data['to_vid']);
			}else{
				$query1 = mysqli_query($con,'SELECT * FROM `' . TB_PREFIX . 'odata` WHERE `wref` = ' . $data['to_vid']);
			}
			$data1 = mysqli_fetch_assoc($query1);
			$query2 = mysqli_query($con,'SELECT * FROM `' . TB_PREFIX . 'users` WHERE `id` = '.$data1['owner']);
			$data2 = mysqli_fetch_assoc($query2);
			$query11 = mysqli_query($con,'SELECT * FROM `' . TB_PREFIX . 'vdata` WHERE `wref` = '.$getFLData['wref']);
			$data11 = mysqli_fetch_assoc($query11);
			$query21 = mysqli_query($con,'SELECT * FROM `' . TB_PREFIX . 'users` WHERE `id` = '.$data11['owner']);
