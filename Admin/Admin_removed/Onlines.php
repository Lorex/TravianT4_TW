...
<?php
	$time = time() - (60*5);
	$sql = mysqli_query($con,"SELECT * FROM ".TB_PREFIX."users where timestamp > $time and id > 3 ORDER BY username ASC $limit");
	...
	$time = time() - (60*5);
	$sql2 = mysqli_query($con,"SELECT * FROM ".TB_PREFIX."users where timestamp > $time and id > 3 ORDER BY username ASC $limit");

if($query>0){
	while($row = mysqli_fetch_array($sql2)){
		$uid = $row['id'];
		$sql3 = mysqli_query($con,"SELECT * FROM ".TB_PREFIX."vdata where owner = $uid");
